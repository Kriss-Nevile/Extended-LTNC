...
class RouletteWheelSelection(SelectionStrategy):
    ...
    def select(self, population: List[Chromosome], num_parents: int) -> List[Chromosome]:
        ...
        total_fitness = sum(c.fitness for c in population if c.fitness is not None)
        
        if total_fitness <= 0:
            # If all fitness values are 0 or negative, use uniform random selection
        ...
        for _ in range(num_parents):
            ...
            for chromosome in population:
                fitness = chromosome.fitness if chromosome.fitness is not None else 0
                current += fitness
                if current >= pick:
                    selected.append(chromosome.copy())
                    break
            else:
                # Fallback: select the last chromosome
